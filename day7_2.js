...
const lines = tachyonManifolds.split("\n");
...
lines.forEach((line) => {
  ...
  if (line.includes("S")) {
    ...
    nextStates[startIndex] = 1;
    currentStates = nextStates;
    return;
  }

  for (const [position, count] of Object.entries(currentStates)) {
    const column = parseInt(position);
    const branch = line[column];

    if (branch === "^") {
      //count ways to left branch
      nextStates[column - 1] = (nextStates[column - 1] || 0) + count;

      //count ways to right branch
      nextStates[column + 1] = (nextStates[column + 1] || 0) + count;
    }

    if (branch === ".") {
      //keep count for this way, since it doesnt split here
      nextStates[column] = (nextStates[column] || 0) + count;
    }
  }
  currentStates = nextStates;
});
...
const totalTimelines = Object.values(currentStates).reduce((a, b) => a + b, 0);
